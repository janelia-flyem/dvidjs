...
Connection.prototype.serverInfo = function(callback) {
  var url = this.createUrl('api/server/info');
  var req = create_request({url: url, callback: callback});
  req.send();
}

Connection.prototype.load = function(callback) {
  var url = this.createUrl('api/load');
  var req = create_request({url: url, callback: callback});
  req.send();
}

Connection.prototype.reposInfo = function(callback) {
