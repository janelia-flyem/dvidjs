 ...
 * @param {string} [options.username=anonymous] - The name of the user making the request 
 * @param {string} [options.application=dvidjs] - The name of the application making a call to the dvid server.
 **/
...
Connection.prototype.createUrl = function(path) {
  ...
  var url_string = url.resolve(hostString, path);

  // convert the string into an object so we can modify it.
  url_obj = url.parse(url_string, true);
  ...
  if (options.hasOwnProperty('callback')) {
    xhr.onload = function() {
      if (xhr.status === 200) {
        ...
        // tack the status on to the error object.
